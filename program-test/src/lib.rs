...
pub fn anchor_builtin_process_instruction(
    process_instruction: RestrictiveProcessInstruction,
...
/// Converts a `solana-program`-style entrypoint into the runtime's entrypoint style, for
/// use with `ProgramTest::add_program` but with more restrictive lifetimes
#[macro_export]
macro_rules! anchor_processor {
    ($process_instruction:expr) => {
        Some(
            |invoke_context, _arg0, _arg1, _arg2, _arg3, _arg4, _memory_mapping, result| {
                *result = $crate::anchor_builtin_process_instruction($process_instruction, invoke_context)
                    .map(|_| 0)
                    .into();
            },
        )
